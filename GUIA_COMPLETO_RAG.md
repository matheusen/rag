...
14. [RAG vs Fine-tuning vs Prompt Engineering vs DSPy/GEPA](#14-rag-vs-fine-tuning-vs-prompt-engineering-vs-dspygepa)
...
A literatura define três gerações de sistemas RAG com complexidade crescente, mais arquiteturas especializadas que emergiram na prática.
...
### 7.3 Arquiteturas Especializadas

Além das gerações clássicas, quatro arquiteturas resolvem problemas específicos de produção.

#### 7.3.1 Conversational RAG (com memória)

Resolve a "cegueira contextual": em RAG padrão, perguntas de acompanhamento como "Quanto custa?" falham porque o sistema não sabe a que "isso" se refere.

```
[Histórico da conversa: últimas 5–10 interações]
         ↓
[Reescrita de query com contexto]
  "Você pode redefini-la?" → "Você pode redefinir minha chave de API?"
         ↓
[Busca vetorial com query reescrita]
         ↓
[Geração com contexto da conversa]
```

**Prós:**
- Experiência de chat natural e multi-turno
- Impede que o usuário tenha que se repetir

**Contras:**
- Desvio de memória: contexto irrelevante antigo pode contaminar a busca atual
- Custo de tokens mais alto pela etapa de reescrita

**Quando usar:** chatbots de suporte, assistentes de atendimento, qualquer interface conversacional.

---

#### 7.3.2 Corrective RAG — CRAG (autoverificação)

Introduz um "Portão de Decisão" que avalia a qualidade dos documentos recuperados *antes* de enviá-los ao gerador. Se a busca interna for insuficiente, aciona fallback para fontes externas.

```
Query → Retrieval → [Classificador de qualidade]
                         ↓
               Correto ──┤── Ambíguo ──┬── Incorreto
                  ↓      │             │       ↓
              Gerador     │         Filtra   Busca web
                          ↓             │   (Tavily, etc.)
                       Gerador     Descarta        ↓
                                              Gerador
```
...
- **Correto:** prossegue para o gerador
- **Ambíguo:** filtragem adicional
- **Incorreto:** descarta e aciona API externa

**Prós:**
- Reduz drasticamente alucinações
- Preenche a lacuna entre dados internos e fatos em tempo real

**Contras:**
- Acrescenta 2–4 segundos de latência quando o fallback é acionado
- Gerenciamento de custos e rate limits de APIs externas

**Quando usar:** domínios de alto risco (financeiro, médico, jurídico), consultas sobre informações dinâmicas.

---

#### 7.3.3 Adaptive RAG (roteamento por complexidade)

Usa um classificador de intenção para escolher o caminho mais eficiente, evitando processamento desnecessário para queries simples.

```
Query → [Classificador de complexidade]
              ↓
    ┌─────────┼─────────┐
    ↓         ↓         ↓
Caminho A  Caminho B  Caminho C
(sem busca) (RAG      (multi-step
            simples)   agêntico)
    ↓         ↓         ↓
 Resposta  Resposta  Resposta
  direta   padrão    complexa
```

**Caminhos:**
- **A (sem busca):** saudações, conhecimento geral que o LLM já possui
- **B (RAG simples):** perguntas factuais diretas — busca única
- **C (multi-step agêntico):** perguntas analíticas complexas que exigem múltiplas fontes

**Prós:**
- Redução significativa de custo (80% das queries vão para A ou B)
- Latência ótima para queries simples

**Contras:**
- Risco de classificação incorreta: pergunta difícil tratada como fácil → falha silenciosa
- Requer modelo de roteamento bem calibrado

**Quando usar:** sistemas com volume alto de queries e perfil misto de complexidade.

---

#### 7.3.4 Fusion RAG (múltiplos ângulos)

Resolve o "problema da ambiguidade": gera 3–5 variações da query, busca todas em paralelo, e combina os resultados por Reciprocal Rank Fusion (RRF).

```
Query original
      ↓
[Expansão de queries] → query_1, query_2, query_3, query_4, query_5
      ↓
[Busca paralela para todas as variações]
      ↓
[RRF: documentos que aparecem no topo de múltiplas buscas sobem]
      ↓
[Top-K documentos ranqueados por consenso → LLM]
```
...
**Prós:**
- Alta cobertura — encontra documentos que uma única query não detectaria
- Resistente a queries mal formuladas pelo usuário

**Contras:**
- Multiplica o custo de busca (3x–5x)
- Maior latência devido aos cálculos de reranqueamento

**Quando usar:** bases de conhecimento com terminologia variada, pesquisa técnica ou científica.

---

### 7.4 Modular RAG (RAG Modular)
...
### 7.5 Agentic RAG (RAG Agêntico) — Geração Atual
...
### 8.2 Busca Esparsa — BM25 e TF-IDF

Algoritmos clássicos de recuperação de informação baseados em frequência de termos.

**TF-IDF** é a base conceitual: pondera cada termo pela frequência no documento (TF) e pela raridade no corpus (IDF). Usa cosine similarity entre vetores esparsos para ranquear documentos.

```
TF-IDF(t, d) = TF(t, d) × IDF(t)
             = (freq do termo no doc) × log(N_docs / docs_com_o_termo)
```

**BM25** é a evolução do TF-IDF com dois ajustes práticos: saturação da frequência (um termo muito comum não cresce indefinidamente) e normalização pelo tamanho do documento.

```
BM25(query, doc) = Σ IDF(t) × (tf(t,d) × (k₁+1)) / (tf(t,d) + k₁ × (1 - b + b × |d|/avgdl))
```

**Vantagem de ambos:** Excelentes para termos técnicos, siglas, nomes próprios, código.
...
Combina busca densa + BM25/TF-IDF. Os scores são fundidos por Reciprocal Rank Fusion (RRF).
...
#### Quando GraphRAG vs. RAG vetorial
...
### 9.2 Verificação de Fatos (Fact-Checking)
...
Baseado nos papers analisados, estas são as técnicas com maior evidência empírica de eficácia.
...
**5. Adaptive RAG (roteamento por complexidade)**
- O que fazer: classificar a query e escolher entre sem busca / busca simples / multi-step
- Ganho: redução de 60–80% no custo total ao evitar recuperações desnecessárias
- Custo de implementação: baixo-médio — requer classificador calibrado
- Por que funciona: a maioria das queries é simples e não precisa do pipeline completo

### Tier 2 — Alto impacto, implementação moderada

**6. Recuperação Iterativa com Gap Analysis (FAIR-RAG)**
...
- O que fazer: avaliar qualidade dos docs recuperados antes de gerar; acionar fallback na web se ruins
- Ganho: redução drástica de alucinações em domínios de alto risco
- Custo: médio — requer modelo classificador + integração com API externa

**8. Diversidade na Recuperação (Vendi-RAG)**
...
**9. MCTS + RAG para raciocínio (RAG-Star)**
...
**10. Compressão de contexto no decoder (REFRAG)**
- O que fazer: comprimir chunks em embeddings antes de enviar ao decoder
- Ganho: 30× de aceleração de inferência
- Custo: alto — requer continual pre-training do modelo

**11. Agentic RAG com DPO de processo (RAG-Gym)**
...
**12. GEPA + DSPy — Otimização Automática de Prompts**
- O que fazer: usar GEPA para evoluir prompts automaticamente em vez de ajuste manual
- Ganho: 35× mais eficiente que MIPROv2, prompts 9× menores com 10% mais performance
- Custo: baixo-médio — requer dataset de avaliação, sem retreinar o modelo
- Por que funciona: GEPA usa algoritmos genéticos + Pareto optimization para explorar o espaço de prompts; aprende com feedback textual dos próprios erros
...
- Solução: REFRAG (30× de aceleração) ou cache de KV pré-computado
...
## 14. RAG vs Fine-tuning vs Prompt Engineering vs DSPy/GEPA

Quatro formas de especializar um LLM, com trade-offs distintos.

```
                    Prompt Eng.   DSPy/GEPA      RAG         Fine-tuning
                         ↓            ↓           ↓               ↓
Conhecimento         Contextual   Contextual   Externo (docs)  Paramétrico
Atualização          Imediata     Imediata     Imediata        Retreinamento
Custo inferência     Baixo        Baixo        Médio           Baixo (após treino)
Custo setup          Baixo        Baixo–Médio  Médio           Alto
Explicabilidade      Média        Média        Alta (fontes)   Baixa
Otimização auto      Não          Sim          Não             Não
Privacidade          Média        Média        Alta            Média
```

### DSPy/GEPA — A Quarta Via

**DSPy** (Declarative Self-improving Language Programs) trata o LLM como um dispositivo programável. Em vez de escrever prompts manualmente, você declara *Language Signatures*:

```python
# Em vez de:
prompt = "Dado o contexto abaixo, responda a pergunta de forma concisa..."
...
O framework compila, otimiza e gera os prompts automaticamente com base nos dados de avaliação.

**GEPA (Genetic-Pareto Prompt Optimizer)** vai além: usa algoritmos genéticos para *evoluir* os prompts ao longo do tempo.

- Gera variações dos prompts mais eficazes (crossover genético)
- Mantém múltiplos prompts via Pareto optimization — nenhum vencedor único
- Aprende com feedback textual dos erros, não apenas com recompensas escalares
- Constrói uma árvore de evolução: cada geração acumula melhorias das anteriores

```
Geração 1: prompt_base
Geração 2: prompt_base + melhoria_A → prompts_melhorados
Geração 3: prompt_melhorado + melhoria_B → prompts_ainda_melhores
...
Geração N: prompts otimizados automaticamente
```

**Resultado empírico:** 35× mais eficiente que MIPROv2, prompts 9× menores com 10% mais performance.

**Diferença fundamental em relação ao RL tradicional:**
- RL usa recompensa escalar ("correto/incorreto") → aprendizado ineficiente
- GEPA usa feedback textual rico ("parcialmente correto, faltou mencionar X") → aprendizado direcionado
...
| Otimizar prompts automaticamente sem retreinar | DSPy/GEPA |
...
- RAG padrão seleciona por similaridade — por design, ignora documentos pouco similares à query.
- Long context lê tudo, mas custa muito.
- A solução é o padrão **Map-Reduce Hierárquico**.
...
Custo: ~$0,002. Rápido. Pode perder um documento não recuperado.
```
...
**Agentic RAG com RL** substitui pipelines estáticos. O agente aprende quando e como buscar via reinforcement learning. DPO com supervisão de processo é a técnica dominante.

**MCTS + RAG** conecta retrieval ao paradigma de test-time compute. Em vez de um único passo de geração, o sistema explora múltiplos caminhos de raciocínio, verificados por RAG.

**Compressão de contexto** (REFRAG) torna-se infraestrutura padrão. À medida que contextos crescem, a compressão de chunks no decoder se torna economicamente necessária.

**GraphRAG e Hypergraphs** substituem busca vetorial pura para domínios com estrutura relacional forte.

**RAG multimodal** estende o paradigma para imagens, vídeo, áudio e dados estruturados (tabelas).

**GEPA/DSPy como infraestrutura de prompts** — a tendência é que otimização automática de prompts se torne padrão, da mesma forma que compiladores substituíram a programação em assembly. Sistemas que gerenciam seus próprios prompts adaptativamente serão mais robustos do que sistemas com prompts estáticos escritos manualmente.

**RAG quântico** ainda é teórico, mas a busca de Grover oferece aceleração quadrática teórica.
...
2024: Advanced RAG; Multi-hop RAG; RAG-Star com MCTS; Adaptive RAG; CRAG
2025: Agentic RAG com RL; REFRAG (Meta); DSPy 3 + GEPA; Conversational RAG
2026: GraphRAG enterprise; RAG multimodal; Hypergraph RAG
2027+: RAG integrado nativamente em LLMs? Context engineering?
```

### A questão central — Long Context vs RAG

Modelos como Gemini 1.5 Pro (1M tokens) colocam uma pergunta direta na mesa: se você pode colocar tudo no contexto, por que construir RAG?
...
**As razões pelas quais RAG ainda vence na maioria dos casos:**
...
6. **Explicabilidade:** RAG cita a fonte de cada afirmação. Com long context alucinando, não há rastreamento.
...
- **DSPy/GEPA**: candidato para a próxima iteração — quando houver dataset de avaliação suficiente para alimentar a otimização automática de prompts.
...
| Retriever | Busca chunks relevantes | HNSW, BM25, TF-IDF, híbrido |
| Reranker | Reordena por relevância | cross-encoder, Cohere |
| LLM Generator | Gera resposta com contexto | Gemini, GPT-4, Llama |
| Prompt Optimizer | Otimiza prompts automaticamente | DSPy, GEPA |
| Evaluator | Mede qualidade do sistema | Ragas, LLM-as-judge |

---

*Documento baseado em 42+ artigos científicos publicados entre 2020 e 2026, incluindo pesquisas de Meta AI, Google, Microsoft, IBM, Amazon, KAIST, Renmin University, Stanford, MIT e outros, complementado por artigos de referência sobre arquiteturas RAG (Divy Yadav, 2025) e DSPy 3 + GEPA (Gao Dalie, 2025).*
